alter table public.session_pointage
add column if not exists last_seen_session_pointage timestamp with time zone null;

alter table public.pause_pointage
add column if not exists last_seen_pause_pointage timestamp with time zone null;

update public.session_pointage
set last_seen_session_pointage = coalesce(last_seen_session_pointage, debut_session_pointage)
where last_seen_session_pointage is null;

update public.pause_pointage
set last_seen_pause_pointage = coalesce(last_seen_pause_pointage, debut_pause_pointage)
where last_seen_pause_pointage is null;

create or replace function public.rpc_pointage_heartbeat(
  p_user_id integer
)
returns table (
...
begin
  ...
  set last_seen_session_pointage = v_now
  where s.id_session_pointage = v_session_id;

  select pp.id_pause_pointage
  into v_pause_id
  ...
  if v_pause_id is not null then
    ...
    set last_seen_pause_pointage = v_now
