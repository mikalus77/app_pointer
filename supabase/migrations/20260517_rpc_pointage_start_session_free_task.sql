create or replace function public.rpc_pointage_start_session(
  ...
  p_free_task_label text default null
)
returns table (
  id_pointage integer,
...
declare
  ...
  v_free_task_label text;
begin
  perform pg_advisory_xact_lock(71002, p_user_id);

  v_free_task_label := nullif(trim(p_free_task_label), '');
  ...
  where p.id_utilisateur_pointeur = p_user_id
    ...
    and (
      (v_free_task_label is null and p.libelle_tache_libre_pointage is null)
      or p.libelle_tache_libre_pointage = v_free_task_label
    )
